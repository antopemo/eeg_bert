...
from .loader import StockBertConfig, map_stock_config_to_params,map_to_stock_variable_name, load_stock_weights, bert_prefix
...
def albert_params(albert_model: str):
    ...
    if tf.io.gfile.isdir(albert_model):
        ...
        if tf.io.gfile.exists(config_file):
            stock_config = StockBertConfig.from_json_file(config_file)
        else:
        ...
        if albert_model in albert_models_config:                                    # google tfhub v1 weights
            ...
            stock_config = StockBertConfig.from_dict(albert_config, return_instance=True, return_unused=False)
        else:
    ...
    params = map_stock_config_to_params(stock_config)
    return params
...
def load_albert_weights(bert: BertModelLayer, tfhub_model_path, tags=[]):
    ...
    if not _is_tfhub_model(tfhub_model_path):
        ...
        return load_stock_weights(bert, tfhub_model_path, map_to_stock_fn=map_to_stock_fn)
